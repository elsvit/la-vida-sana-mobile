...
export default function Product() {
  ...
  const renderProductInfo = () => {
    ...
    if (product.brand) {
      infoItems.push(
        <View key="brand" style={styles.infoSection}>
          <Text variant="titleMedium" style={styles.sectionTitle}>
  ...
  const renderProductDetails = () => {
    ...
    if (product.quantity) {
      details.push(
        <Chip key="quantity" mode="outlined" style={styles.detailChip}>
    ...
    if (product.packaging) {
      details.push(
        <Chip key="packaging" mode="outlined" style={styles.detailChip}>
          {t('products.packaging')}: {product.packaging}
        </Chip>,
      );
    }

    if (product.size && product.sizeFormat) {
      details.push(
        <Chip key="size" mode="outlined" style={styles.detailChip}>
          {t('products.size')}: {product.size} {product.sizeFormat}
        </Chip>,
      );
    }
    ...
    if (product.seller) {
      details.push(
        <Chip key="seller" mode="outlined" style={styles.detailChip}>
          {t('products.seller')}: {product.seller}
        </Chip>,
      );
    }
  ...
  return (
    <SafeAreaBackground>
      ...
      >
        ...
        {renderProductDetails().length > 0 && (
          <View style={styles.chipsContainer}>{renderProductDetails()}</View>
        )}
